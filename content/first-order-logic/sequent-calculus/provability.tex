...
\begin{thm}
\ollabel{thm:strong-generalization} If $y$ is a variable not occurring in $
\Gamma$ or $\lforall x !A$ and $\Gamma \Proves[\Log{LK}] \Subst{!A}{y}{x}$, 
then $\Gamma \Proves[\Log{LK}] \lforall[x][!A]$.
\end{thm}

\begin{proof}
Let $\Pi_0$ be an $\Log{LK}$-!!{derivation} of $\Gamma_0 \Sequent \Subst{!A}{y
}{x}$ for some finite $\Gamma_0 \subseteq \Gamma$.  By adding a \RightR{
\lforall} inference, we obtain a proof of $\Gamma_0 \Sequent \lforall[x][!A]$
, since $y$ does not occur in $\Gamma$ or $\lforall x !A$ and thus the 
...
\tagitem{prvEx}{If $\Gamma \Proves[\Log{LK}] \Subst{!A}{t}{x}$ then $\Gamma \Proves
  \lexists[x][!A]$.}{}

\tagitem{prvAll}{If $\Gamma \Proves[\Log{LK}] \lforall[x][!A]$ then $\Gamma \Proves
  \Subst{!A}{t}{x}$.}{}
\end{tagenumerate}
\end{thm}
...
\begin{proof}
\begin{tagenumerate}{prvEx,prvAll}
\tagitem{prvEx}{Suppose $\Gamma \Proves[\Log{LK}] \Subst{!A}{t}{x}$. Then for some
  finite~$\Gamma_0 \subseteq \Gamma$, $\Log{LK}$ !!{derive}s $\Gamma_0
  \Sequent \Subst{!A}{t}{x}$. Add an \RightR{\lexists} inference to get
  !!a{derivation} of $\Gamma_0 \Sequent \lexists[x][!A]$.}{}

\tagitem{prvAll}{Suppose $\Gamma \Proves[\Log{LK}] \lforall[x][!A]$. Then there is a
  finite $\Gamma_0 \subseteq \Gamma$ and an $\Log{LK}$-!!{derivation}
  $\Pi$ of $\Gamma_0 \Sequent \lforall[x][!A]$.  Then
% for some reason, prooftree env sometimes doesn't work here
% \begin{prooftree}
\[
\AxiomC{}
\RightLabel{$\Pi$}
\Deduce$\Gamma_0 \fCenter \lforall[x][!A]$
\Axiom$\Subst{!A}{t}{x} \fCenter \Subst{!A}{t}{x}$
\RightLabel{\LeftR{\lforall}}
\UnaryInf$\lforall[x][!A] \fCenter \Subst{!A}{t}{x}$
\RightLabel{cut}
\BinaryInf$\Gamma_0 \fCenter \Subst{!A}{t}{x}$
%\end{prooftree}
\DisplayProof
\]
shows that $\Gamma_0 \Proves[\Log{LK}] \Subst{!A}{t}{x}$.}{}
\end{tagenumerate}
\end{proof}
\end{document}
