...
\begin{ex}
Give an $\Log{LK}$-derivation of the sequent $\lexists[x][\lnot !A]
\Sequent \lnot \lforall[x][!A]$.
...
\begin{prooftree}
\AxiomC{}
\UnaryInf$\lexists[x][\lnot !A] \fCenter \lnot \lforall[x][!A]$
\end{prooftree}
...
later, so we'll do that one first. Indeed, $x$ satifies the eigenvariable
condition because all $x$ on the right side are bound by the $\lforall$
quantifier.
\begin{prooftree}
\AxiomC{}
\UnaryInf$ \Subst{\lnot !A}{x}{x} \fCenter \lnot \lforall[x][!A]$
...
\begin{prooftree}
\AxiomC{}
\UnaryInf$\lforall[x][!A] \fCenter !A$
\Deduce$ \lnot !A \fCenter \lnot \lforall[x] !A$
\RightLabel{\LeftR{\lexists}}
\UnaryInf$ \lexists[x] \lnot !A \fCenter \lnot \lforall[x] !A$
\end{prooftree}
...
eigenvariable condition was \LeftR{\exists}, and the variable~$x$
does not occur free in its lower sequent (the end-sequent), this is a
correct derivation.
\end{ex}
